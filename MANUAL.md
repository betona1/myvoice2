...
# 로그인 (한 번)
curl -ksS -c $COOKIE -X POST https://YOUR_SERVER:9092/api/auth/login \
  -d "email=admin@myvoice2.local" -d "password=$PW"

# 학습 시작
curl -ksS -b $COOKIE -X POST https://YOUR_SERVER:9092/api/finetune/start \
  -d "voice_ids=3,4,5,6,9,10,11,12" \
  -d "model_name=noh_v1" \
  -d "epochs=10" -d "batch_size=2"

# 진행 상태 폴링
curl -ksS -b $COOKIE https://YOUR_SERVER:9092/api/finetune/status

# 모델 목록
curl -ksS -b $COOKIE https://YOUR_SERVER:9092/api/finetune/models

# 학습된 모델로 합성
curl -ksS -b $COOKIE -X POST https://YOUR_SERVER:9092/api/tts/generate-finetuned \
...
### 마이크 안 됨
브라우저는 secure context(HTTPS, localhost만)에서만 마이크 허용. `https://YOUR_SERVER:9092`로 접속하고 자체서명 인증서 경고 통과.
